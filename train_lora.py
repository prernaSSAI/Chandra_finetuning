...
try:
    from unsloth.trainer import UnslothVisionDataCollator #Raw data → model input format
...
class LoRAConfig:
    r: int = 16                          # LoRA rank
    lora_alpha: int = 16                 # LoRA alpha (recommended: >= r)
...
# SECTION 4 — REPRODUCIBILITY & DEVICE
...
# SECTION 6 — TRAIN / VALID / TEST SPLIT
...
def split_dataset(
    ...
    # Save exact page membership so evaluation can reproduce the same split
    "train_pages": [
        {
            "file": sample["file"],
...
# SECTION 7 — CONVERT TO HF DATASET

def samples_to_hf_dataset(samples: List[Dict]) -> "HFDataset": #converts raw Python samples into a HuggingFace Dataset that collator expects,
...
def load_model_and_processor(
    ...
    # Step 2: attach LoRA adapters — ALL layers enabled 
    log.info(
...
def main(cfg: TrainingConfig) -> None:
    ...
    sft_cfg = SFTConfig(
        ...
        # max_steps = 30,  # uncomment this (and comment num_train_epochs) for quick test runs
        ...
        # Evaluation — one pass per epoch
        eval_strategy                = "epoch",
        save_strategy                = "best",        # only save when val loss improves
        load_best_model_at_end       = True,
        metric_for_best_model        = "eval_loss",
        greater_is_better            = False,

        # Vision fine-tuning MUST-HAVES (from Unsloth notebook)
...
if __name__ == "__main__":
    cfg = TrainingConfig(
        ...
        lora=LoRAConfig(
            ...
            finetune_vision_layers     = True,   # ← all layers ON (no freezing)
