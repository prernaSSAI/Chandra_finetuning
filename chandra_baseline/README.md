...
### Step 4: Sanity-Test CER / WER / TEDS With Manual Errors

Before trusting a full model evaluation run, do a small offline metric sanity
test. This does not train the model and does not run inference. It only checks
whether the metric code reacts correctly when you manually introduce errors.

Create an editable template:

```bash
python chandra_baseline/manual_metric_sanity_check.py \
  --make-template chandra_baseline/metric_sanity/manual_metric_cases.json
```

Open `chandra_baseline/metric_sanity/manual_metric_cases.json` and edit the
cases. Keep `ground_truth` as the correct annotation and make controlled
mistakes in `prediction`, for example:

* Change one character or number to verify CER changes.
* Change or remove one word to verify WER changes.
* Remove a table row, table cell, `rowspan`, or `colspan` to verify TEDS drops.
* Keep one exact-match case to verify CER=0, WER=0, and TEDS=1.

Run the metric check:

```bash
python chandra_baseline/manual_metric_sanity_check.py \
  --input chandra_baseline/metric_sanity/manual_metric_cases.json \
  --output chandra_baseline/metric_sanity/manual_metric_results.json \
  --csv-output chandra_baseline/metric_sanity/manual_metric_results.csv
```

Expected behavior:

* Exact match: `cer = 0`, `wer = 0`, `teds = 1`.
* Text-only mistakes: `cer` and `wer` increase; `teds` may stay high if table
  structure is unchanged.
* Table-structure mistakes: `teds` decreases.
* Empty prediction: `cer` and `wer` are high, and `teds = 0`.

If TEDS fails because dependencies are missing, install the baseline
requirements in the same environment:

```bash
pip install beautifulsoup4 lxml apted jiwer
```
